...
export BROADCAST_GROUPS="$(get_env BROADCAST_GROUPS)"
export BOT_PREFIX="$(get_env BOT_PREFIX)"
export BOT_ADMINS="$(get_env BOT_ADMINS)"
...
for f in ototext-bot-commands.json ototext-bot-worker.json scenario-1-scheduled-group-broadcast.json scenario-2-sync-group-participants.json; do
  "$N8N_BIN" import:workflow --input="$ROOT_DIR/n8n-workflows/$f"
done
